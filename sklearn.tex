...
One needs to \mintinline{python}!fit! to learn the parameters and then \mintinline{python}!transform!, or equivalently, use the convenience method \mintinline{python}!fit_transform!:
\begin{minted}{python}
from sklearn.preprocessing import StandardScaler
X_scaled = StandardScaler().fit(X).transform(X) 
X_scaled = StandardScaler().fit_transform(X) # same as the above
\end{minted}
We quickly remark that the above method chaining implies that the methods have\footnote{actually \mintinline{python}!fit! takes an optional \mintinline{python}!y! input for supervised learning. Estimators normally also inherit from \mintinline{python}!BaseEstimator!, which we say again later.} the signatures
\begin{minted}{python}
class MyEstimator:
# ...
